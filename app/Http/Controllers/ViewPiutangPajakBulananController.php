...
class ViewPiutangPajakBulananController extends Controller
{
    public function jenis($tahun)
    {
        $get = JenisPajak::where('tahun', $tahun)->with('transaksi_piutang')->get();
        ...
        foreach ($get as $key => $value) {
            ...
            foreach ($value->transaksi_piutang as $trx) {
                if ($trx->bulan == 1) {
                    $jan += $trx->jumlah;
                }
                if ($trx->bulan == 2) {
        ...
        return view('pages.piutang-bulanan.jenis')
            ->with('get', $get)
            ->with('data', $data)
            ->with('ls', $ls)
    ...
    public function kategori($id_jenis_pajak)
    {
        ...
        $get = KategoriPajak::where('id_jenis_pajak', $id_jenis_pajak)->with('transaksi_piutang')->get();
        ...
        return view('pages.piutang-bulanan.kategori')
            ->with('jenis', $jenis)
            ->with('data', $data);
    }
}
