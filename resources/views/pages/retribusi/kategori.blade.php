...
@section('content')
    <div class="row">
        <div class="col-md-6">
            <h3>Pajak {{ ucwords(strtolower($jp->nama)) }} {{ $tahun }}</h3>
    ...
    <div class="row" style="margin-top: 30px">
        <div class="col-md-{{ $jp->id != 6 ? '6' : '12' }}">
            <h6>Tabel Penerimaan Berdasarkan Kategori Pajak {{ ucwords(strtolower($jp->nama)) }} (Termasuk Piutang)</h6> <hr>
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    @foreach ($kp as $key => $item)
                        <tr>
                            <td>{{ $key = $key + 1 }}</td>
                            <td><a href="{{ route('ret-pajak.detail', $item->id) }}">{{ $item->nama }}</a></td>
